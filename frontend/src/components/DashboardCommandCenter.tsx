...
import { ThemeMode, CaseFile, RiskLevel } from '../types';
...
export const DashboardCommandCenter: React.FC<DashboardCommandCenterProps> = ({
  ...
  const filteredCases = mockCaseQueue.filter((c) => {
  ...
  return (
    <section id="dashboard" className="py-20 px-4 sm:px-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-xs sm:text-sm text-zinc-400 mt-1">
            12 active fraud investigations currently under automated multi-hop analysis.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-indigo-600 hover:from-cyan-400 text-white text-xs font-bold shadow-md shadow-cyan-500/25 flex items-center gap-1.5 transition-all cursor-pointer"
          >
      ...
      >
        ...
        <button
          onClick={() => onSelectCase(mockCaseQueue[0])}
